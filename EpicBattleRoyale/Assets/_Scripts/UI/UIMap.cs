...
public class UIMap : MonoBehaviour {
    ...
    void UpdateGridColor (int i, int j, bool active) {
        grids[i, j].color = (active) ? Color.red : Color.black;
    }
    ...
    void CreateLineRenderers () {
        grids = new Image[MapsController.Ins.mapSize, MapsController.Ins.mapSize];
        for (int i = 0; i < MapsController.Ins.mapSize; i++) {
            for (int j = 0; j < MapsController.Ins.mapSize; j++) {
    ...
    void CreateLineRenderer (MapsController.MapInfo data, Vector2Int coords) {
        ...
        grids[coords.x, coords.y] = newLr.GetComponent<Image> ();
        UpdateGridColor (coords.x, coords.y, MapsController.Ins.curMapCoords == coords);
